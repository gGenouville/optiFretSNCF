...
from module.utils import base_time
...
def visualisation_gantt(t_arr: dict, t_dep: dict) -> Figure:
    ...
    tasks = [
        {
            ...
            "Finish": base_time(1)
            + datetime.timedelta(minutes=15 * var_arr.X + Taches.T_ARR[m_arr]),
    ...
    ] + [
        {
            ...
            "Finish": base_time(1)
            + datetime.timedelta(minutes=15 * var_dep.X + Taches.T_DEP[m_dep]),
    ...
    sorted_resources = list(
        itertools.chain.from_iterable(
            [sorted(resource_per_machine[m]) for m in ordered_machines]
        )
    )
    ...
    fig.update_layout(
        xaxis=dict(title="Temps", tickformat="%d/%m/%y %H:%M"),
    )
    fig.update_yaxes(
        categoryorder="array",
