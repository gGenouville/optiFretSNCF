...
def visualisation_gantt(t_arr: dict, t_dep: dict) -> Figure :
    ...
    tasks = [
            {"Train": n_arr,
            "Start": base_time(1) + datetime.timedelta(minutes=5*var_arr.X),
            "Finish": base_time(1) + datetime.timedelta(minutes=5*var_arr.X + Taches.T_ARR[m_arr]),
            "Machine": f"arr_{m_arr}",
            "Tâches": f"arr_{m_arr}"}
            for (m_arr, n_arr), var_arr in t_arr.items()
        ] + [
            {"Train": n_dep,
            "Start": base_time(1) + datetime.timedelta(minutes=5*var_dep.X),
            "Finish": base_time(1) + datetime.timedelta(minutes=5*var_dep.X + Taches.T_DEP[m_dep]),
